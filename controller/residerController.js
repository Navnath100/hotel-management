...
async function getResiders(req, res, next) {
    ...
    try {
        ...
        const utcStart = new Date(new Date(new Date().setHours(00, 00, 00)).toUTCString()).getTime() - (330 * 60000);
        const utcEnd = new Date(new Date(new Date().setHours(23, 59, 59)).toUTCString()).getTime() - (330 * 60000);
        ...
        result["AC"] = await Resider.find({ $and: [search, { isAC: true }] }).countDocuments();
        result["nonAC"] = await Resider.find({ $and: [search, { isAC: false }] }).countDocuments();
        let page;
        let limit;
        ...
        if (user && user.status == "Active") {
            Resider.findOne({ $and: [{ _id: residerID }] }).then((resider) => {
                if (resider && resider.status == "checked-in") {
                    ...
                    //Checking if guest check in and checked out on same day
